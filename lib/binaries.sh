install_yarn() {
  ...
  if ! read number url < <(curl --get  --connect-timeout $CURL_CONNECT_TIMEOUT --max-time $CURL_TIMEOUT --data-urlencode "range=$version" "https://nodebin.herokai.com/v1/yarn/$platform/latest.txt"); then
    fail_bin_install yarn $version;
  fi

  echo "Downloading and installing yarn ($number)..."
  local code=$(curl "$url" -L --fail  --connect-timeout $CURL_CONNECT_TIMEOUT --max-time $CURL_TIMEOUT  -o /tmp/yarn.tar.gz --write-out "%{http_code}")
  if [ "$code" != "200" ]; then
...
install_iojs() {
  ...
  if ! read number url < <(curl --silent --get --connect-timeout $CURL_CONNECT_TIMEOUT --max-time $CURL_TIMEOUT --data-urlencode "range=$version" "https://nodebin.herokai.com/v1/iojs/$platform/latest.txt"); then
    fail_bin_install iojs $version;
  fi

  echo "Downloading and installing iojs $number..."
  local code=$(curl "$url" --silent --fail --connect-timeout $CURL_CONNECT_TIMEOUT --max-time $CURL_TIMEOUT -o /tmp/iojs.tar.gz --write-out "%{http_code}")
  if [ "$code" != "200" ]; then
